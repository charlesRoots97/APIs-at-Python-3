    ...
    try:
        ...
        if request.method == 'POST':
            try:
                ...
                "WHERE tbl_empresa.id_empresa = 1 AND tbl_solicitud.fecha_captura BETWEEN %s AND %s ")
                # query ='SELECT * FROM tbl_solicitud LIMIT 1'
                cursor.execute(query,(dateInit,dateFinal,))
